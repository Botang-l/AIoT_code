...
class TimeSeriesData:    
    def __init__(self, model_name, start_date, end_date): # 初始化   
        self.__Train_seq = None
        self.__Val_seq = None
        self.__Test_seq = None
        self.__train_max = None
        self.__train_min = None
        self.__first_seq = None
        self.preprocess_seq(model_name, start_date, end_date)
    
    def preprocess_dataset(self, seq_len, model_name, start_date, end_date):
        """
    ...
    def create_seq(self, input_size, seq_len, data):
        """
        ...
        (self.__first_seq, label) = final_seq[0]
        print("示範:")
        print("seq:", self.__first_seq)
        print("label:", label, "\n----------------------------\n")
        return final_seq


    def preprocess_seq(self, model_name, start_date, end_date):
        """
        ...
        dataset = self.preprocess_dataset(args.seq_len, model_name, start_date, end_date)
        ...
        train, self.__train_max, self.__train_min = normalize_data(train, max_min_array)

        # Create sequences based on the specified input size and sequence length
        final_seq = self.create_seq(args.input_size, args.seq_len, train)

        # Batch the training data
        self.__Train_seq = batch_data(final_seq, args.batch_size)

        # Normalize and sequence the validation data
        val, _, _ = normalize_data(val, max_min_array)
        final_seq = self.create_seq(args.input_size, args.seq_len, val)
        self.__Val_seq = batch_data(final_seq, args.batch_size)

        # Normalize and sequence the test data
        test, _, _ = normalize_data(test, max_min_array)
        final_seq = self.create_seq(args.input_size, args.seq_len, test)
        self.__Test_seq = batch_data(final_seq, args.batch_size)
    

    def displayData(self):
        return(self.__Train_seq, self.__Val_seq, self.__Test_seq, self.__train_max, self.__train_min)
    

    def displayFirstData(self):
        return(self.__first_seq)
